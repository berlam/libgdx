...
package com.badlogic.gdx.backends.teavm.emu;

import com.badlogic.gdx.backends.teavm.TeaVMFileHandle;
...
public class PixmapEmulator implements Disposable {
	...
	public static Map<Integer, PixmapEmulator> pixmaps = new HashMap<Integer, PixmapEmulator>();
	static int nextId = 0;
	...
	private void image (HTMLCanvasElement image, int srcX, int srcY, int srcWidth, int srcHeight, int dstX, int dstY, int dstWidth,
		int dstHeight) {
		if (blending == Blending.None) {
			context.setFillStyle(clearColor);
			context.setStrokeStyle(clearColor);
